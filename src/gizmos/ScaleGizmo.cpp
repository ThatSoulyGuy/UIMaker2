...
ScaleGizmo::ScaleGizmo(QObject* parent) : Gizmo(parent) { }

QString ScaleGizmo::GetId() const
{
    return QStringLiteral("scale");
}

QString ScaleGizmo::GetDisplayName() const
{
    return tr("Scale");
}

void ScaleGizmo::Draw(const GizmoContext& ctx)
{
    if (!ctx.painter)
        return;
...
GizmoHitResult ScaleGizmo::HitTest(const QPointF& viewPos, const GizmoContext& ctx)
{
    const QRectF viewRect = ctx.itemViewBounds;
    const QPointF center = viewRect.center();
...
QList<GizmoHandle> ScaleGizmo::GetHandles() const
{
    return {
