...
void EntityTreeModel::ConnectNameSignals(UiElement* node)
{
    if (!node)
        return;
    ...
    if (node != root)
    {
        QObject::disconnect(node, &UiElement::NameChanged, this, nullptr);

        QObject::connect(node, &UiElement::NameChanged, this, [this, node](const QString&)
        {
            const QModelIndex idx = GetIndexFromElement(node);
...
Qt::ItemFlags EntityTreeModel::flags(const QModelIndex& index) const
{
    ...
    if (auto* e = static_cast<UiElement*>(index.internalPointer()))
    {
        if (e->IsSlot())
